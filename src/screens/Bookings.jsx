  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        <OverviewChart />
        <CategoryCard onCategorySelect={setCategoryFilter} />
      </div>
...
function StatCard({ label, value, icon, highlight, onClick }) {
  return (
    <div 
      onClick={onClick}
      className={`p-4 rounded-2xl border border-white/5 bg-white/[0.02] flex items-center justify-between group hover:bg-white/[0.04] transition-all cursor-pointer ${highlight && 'ring-1 ring-violet-500/20'}`}
    >
...
function CategoryCard({ onCategorySelect }) {
  const categories = [
        ...
        {categories.map((cat) => (
          ...
          >
            <div className="flex justify-between text-[9px] font-bold uppercase tracking-tight">
              <span className="text-gray-400 group-hover/item:text-white transition-colors">{cat.name}</span>
            ...
            <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
              <div className={`h-full ${cat.color} opacity-60 group-hover/item:opacity-100 transition-opacity`} style={{ width: `${cat.percent}%` }} />
            </div>
          </div>
        ))}
      </div>
