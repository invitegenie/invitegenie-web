...
function FallbackDashboard({ role, user }) {
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header>
        <h1 className="text-4xl font-black text-white tracking-tighter uppercase">Welcome back, {user?.name || "InviteGenie User"}</h1>
        <p className="text-slate-500 mt-1 uppercase font-bold tracking-widest text-[10px]">Role: {String(role || "FREE").replace("_", " ")}</p>
      </header>
